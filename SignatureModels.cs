...
namespace MinsPDFViewer
{
    ...
    public class SignatureConfig
    {
        ...
        public required AsymmetricKeyParameter PrivateKey { get; set; } 
        ...
        public string? VisualStampPath { get; set; } 
    }

    // [신규] 서명 검증 결과 모델
    public class SignatureValidationResult
    {
        public bool IsValid { get; set; }           // 무결성 검증 결과 (해시 일치 여부)
        public string SignerName { get; set; } = "";// 서명자 이름 (CN)
        public DateTime SigningTime { get; set; }   // 서명 시각
        public string Reason { get; set; } = "";    // 사유
        public string Location { get; set; } = "";  // 위치
        public string Message { get; set; } = "";   // 검증 메시지 (에러 내용 등)
        public bool IsDocumentModified { get; set; }// 문서 변조 여부
    }
}